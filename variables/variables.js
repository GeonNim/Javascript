// 1. var 변수 선언과 호이스팅
// - 2015년 후에 const와 let으로 대체된 변수 선언 키워드
// - 호이스팅 : 변수가 선언되는 순간 해당 데이터가 가장 위에서 선언된 것으로 간주하는 특징을 말한다
// - 자바스크립트는 위에서 아래로 코드를 실행하는 인터프리터 언어다.
//   - 따라서 사용할 변수나 함수는 사용할 위치보다 위에 있는 것이 자연스럽다.

console.log(a);
var a = 1;

// 2. 변수
// - 임의로 만든 식별자에 특정 데이터를 저장하는 것을 말한다
// - 변수가 선언되면 자바스크립트 엔진은 특정 공간(RAM)을 만들고 데이터를 참조한다.

const b = 2;

// 3. const 상수 선언
// 상수는 항상 같은 수로 한번 이름이 정해지면 바꿀 수 없다.
// 상수는 한 번만 저장되기 때문에 별도의 공간을 할당하지 않아 효율적이다.
// 따라서 대부분 값의 할당은 상수로 선언된다.
// console.log(c); // error : 선언전에 접근불가

const c = 4;
// const c = 5;
// c = 5;
console.log(c);

// 4. let 변수 선언
// - 변할 수 있는 값으로 런타임으로 값의 변경(재할당)이 가능하다
// - let으로 선언된 변수는 값의 변경은 가능하지만 선언을 다시할 수 없다
// console.log(d); // error :선언전에 접근불가

let d = 5;
d = 7; //재할당가능
console.log(d);

let sum = 0;
for (let m = 1; m < 100; m++) {
  sum += m;
}
console.log(sum);
